      ...
      if (options.limitAttempts) {
        ...
        user.save(function(saveErr) {
          ...
          if (user.get(options.attemptsField) >= options.maxAttempts) {
            ...
            return cb(null, false, new errors.IncorrectPasswordError(`${options.errorMessages.IncorrectPasswordError}. You have ${options.maxAttempts - user.get(options.attemptsField)} attempts remaining until account lockout!`));
          }
        });
      } else {
